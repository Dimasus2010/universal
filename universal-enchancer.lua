...
-- Остальные функции (NOCLIP, ESP, etc.) остаются такими же как в предыдущем скрипте
-- NOCLIP SYSTEM
...
local flySpeedSlider = createSlider("Fly Speed", 350, 10, 200, 50, function(value)
    ...
    if settings.fly then
        toggleFly(true) -- Перезапускаем fly с новой скоростью
    end
end)
...
local walkSpeedSlider = createSlider("Walk Speed", 410, 16, 100, 50, function(value)
    ...
    if settings.speed then
        toggleSpeed(true) -- Перезапускаем speed с новой скоростью
    end
end)
local jumpPowerSlider = createSlider("Jump Power", 470, 50, 200, 100, function(value)
    settings.jumpPower = value
    if settings.jump then
        toggleJump(true) -- Перезапускаем jump с новой силой
    end
end)
...
-- Hotkey: Right Shift для показа/скрытия GUI
...
print("🎮 Universal Enhancer with Sliders Loaded!")
print("🎯 Hotkeys: Right Shift - Show/Hide GUI")
print("📊 Sliders: Fly Speed, Walk Speed, Jump Power")
print("📱 Works on both PC and Mobile!")